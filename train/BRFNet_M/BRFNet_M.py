...
OUTPUT_DIR = r".save_fixsmall_chlast-plus"
...
class RepVGGBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, k: int = 3, stride: int = 1, deploy: bool = False):
        ...
        self.act = nn.GELU()
        if self.deploy:
            self.rbr_reparam = nn.Conv2d(self.in_channels, self.out_channels, kernel_size=self.k, stride=self.stride, padding=self.padding, bias=True)
            self.rbr_dense = None
            self.rbr_1x1 = None
...
class RingBandFilterNetImageNet(nn.Module):
    def __init__(
        ...
        Cs: Tuple[int, int, int, int] = (64, 128, 256, 512),
        reses: Tuple[int, int, int, int] = (56, 28, 14, 7),
        layerscale_init: float = 1e-3,
        beta_init: float = 0.0,
        stage_mixers: Tuple[str, str, str, str] = ("G", "G", "G", "G"),
        stage_gates: Tuple[bool, bool, bool, bool] = (False, False, False, False),
        local_k: int = 3,
    ):
        super().__init__()
...
def train_one_epoch(
    ...
    for it, (samples, targets) in enumerate(data_loader):
        ...
        if do_step:
            gn_pre = global_grad_norm(model.parameters())

            if float(clip_grad) > 0:
                if str(clip_mode).lower() == "agc":
                    adaptive_clip_grad_(model.parameters(), clip_factor=float(clip_grad))
                else:
                    torch.nn.utils.clip_grad_norm_(model.parameters(), float(clip_grad))

            gn_post = global_grad_norm(model.parameters())
...
def main():
    ...
    model = RingBandFilterNetImageNet(
    ...
    param_groups = param_groups_weight_decay(model, float(WEIGHT_DECAY))
    try:
        optimizer = torch.optim.AdamW(param_groups, lr=float(base_lr), betas=OPT_BETAS, eps=float(OPT_EPS), fused=True)
    except TypeError:
        optimizer = torch.optim.AdamW(param_groups, lr=float(base_lr), betas=OPT_BETAS, eps=float(OPT_EPS))
    ...
    for epoch in range(int(EPOCHS)):
        ...
        do_eval = (int(EVAL_FREQ) <= 1) or ((epoch % int(EVAL_FREQ)) == 0) or (epoch == int(EPOCHS) - 1)
        test_stats = {"main_acc1": float("nan"), "main_acc5": float("nan")}

        if do_eval:
            main_stats = evaluate_one(loader_val, model, device, amp_enabled=amp_enabled, amp_dtype=amp_dtype)
            ...
            print(f"Max accuracy: main={max_acc_main:.2f}%")
        ...
        if (epoch % int(SAVE_LAST_FREQ) == 0) or (epoch == int(EPOCHS) - 1):
